...
export async function memoryEntryMergeHandler(req: Request, res: Response) {
  try {
    ...
    if (!userId || !Array.isArray(timestamps) || !timestamps.length) return res.status(400).json({ ok: false, error: 'Missing userId/timestamps[]' });
    const title = String(mergeTitle || 'Merged Notes');
    const db = new Firestore();
    const coll = db.collection('memory_entries').doc(String(userId)).collection('entries');
    const docs = await Promise.all(timestamps.map((ts: any)=> coll.doc(String(ts)).get()));
    const lines: string[] = [];
    for (const d of docs) {
...
');
    ...
    return res.json({ ok: true, id: (data as any).id, name: (data as any).name, webViewLink: (data as any).webViewLink });
  } catch (e: any) {
    const status = e?.response?.status || 500;
    return res.status(status).json({ ok: false, error: e?.message || 'memory.entry.merge failed' });
  }
}
