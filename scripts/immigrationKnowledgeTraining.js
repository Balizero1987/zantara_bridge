...
exports.runImmigrationKnowledgeTraining = runImmigrationKnowledgeTraining;
const immigrationKnowledgeBase_1 = __importDefault(require("../src/services/immigrationKnowledgeBase"));
const modularLanguageEngine_1 = __importDefault(require("../src/core/modularLanguageEngine"));
async function runImmigrationKnowledgeTraining() {
    ...
    try {
        ...
        const uploadResult = await immigrationKnowledgeBase_1.default.uploadDocumentsToDrive();
        ...
        await immigrationKnowledgeBase_1.default.indexImmigrationDocuments();
        const stats = immigrationKnowledgeBase_1.default.getStatistics();
        ...
        const trainingResult = await immigrationKnowledgeBase_1.default.generateTrainingDataset();
        ...
        await immigrationKnowledgeBase_1.default.saveTrainingData();
        ...
        const testQueries = [
            { query: 'How to renew KITAS visa?', language: 'en' },
            { query: 'Quali documenti servono per PT PMA?', language: 'it' },
            { query: 'Berapa biaya pajak untuk perusahaan asing?', language: 'id' },
            { query: 'What are the requirements for investment visa?', language: 'en' },
            { query: 'Come aprire una società in Indonesia?', language: 'it' }
        ];
        for (const test of testQueries) {
            console.log(`\n🔍 Testing query: "${test.query}" (${test.language})`);
            const result = await immigrationKnowledgeBase_1.default.queryKnowledgeBase(test.query, test.language);
        ...
        const engineStats = modularLanguageEngine_1.default.getProcessingStats();
...
async function createImmigrationLanguageModule() {
    ...
    const immigrationModule = {
        ...
        process: async (input) => {
            ...
            const kbResult = await immigrationKnowledgeBase_1.default.queryKnowledgeBase(input.text, input.context?.language || 'en');
            ...
            const isImmigrationQuery = immigrationKeywords.some(keyword => input.text.toLowerCase().includes(keyword));
    ...
    modularLanguageEngine_1.default.registerModule(immigrationModule);
    modularLanguageEngine_1.default.setModuleConfig('immigration_specialist', {
        enabled: true,
        weight: 0.9
    });
    // Update processing chain to include immigration module
    const currentModules = modularLanguageEngine_1.default.getAvailableModules();
    const newChain = [
        'detection',
        'enhanced_training',
    ...
    modularLanguageEngine_1.default.updateProcessingChain(newChain);
}
